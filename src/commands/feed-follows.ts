import { deleteFeedFollow, getFeedByURL } from "src/lib/db/queries/feeds";
import {
  createFeedFollow,
...
export async function handlerListFeedFollows(cmdName: string, user: User, _: string) {
...
export async function handlerUnfollow(cmdName: string, user: User, ...args: string[]) {
  ...
  const result = await deleteFeedFollow(user, feed.id);
  if (!result) {
    throw new Error(`Failed to delete feed follow: ${feedURL}`);
  }
